/*! gridster.js - v0.2.1 - 2013-10-28 - * http://gridster.net/ - Copyright (c) 2013 ducksboard; Licensed MIT */ ! function (a) {
    function b(b) {
...
function (a, b, c) {
    ...
    var e = {
        ...
        g = !!("ontouchstart" in b),
        h = {
            start: g ? "touchstart.gridster-draggable" : "mousedown.gridster-draggable",
            move: g ? "touchmove.gridster-draggable" : "mousemove.gridster-draggable",
            end: g ? "touchend.gridster-draggable" : "mouseup.gridster-draggable"
        }, i = d.prototype;
    i.init = function () {
        ...
        var b, c = f.scrollTop(),
            ...
            h = d + 50;
        a.pointer.left;
        var i = d + a.pointer.top,
            j = this.doc_height - this.window_height + this.player_height;
...
function (a, b, c) {
    function d(b, c) {
        this.options = a.extend(!0, e, c), this.$el = a(b), this.$wrapper = this.$el.parent(), this.$widgets = this.$el.children(this.options.widget_selector).addClass("gs-w unselected"), this.widgets = [], this.$changed = a([]), this.wrapper_width = this.$wrapper.width(), this.min_widget_width = 2 * this.options.widget_margins[0] + this.options.widget_base_dimensions[0], this.min_widget_height = 2 * this.options.widget_margins[1] + this.options.widget_base_dimensions[1], this.$style_tags = a([]), this.init()
    }
    var e = {
        namespace: "",
        widget_selector: "li",
    ...
    f.init = function () {
        ...
        var h;
        c || (c = 1), d || (d = 1), !e & !f ? h = this.next_position(c, d) : (h = {
            col: e,
            row: f
        ...
        var c = {
            ...
            max_size_x: parseInt(b.attr("data-max-sizex"), 10) || !1,
            max_size_y: parseInt(b.attr("data-max-sizey"), 10) || !1,
            el: b
        };
        ...
        this.cells_occupied_by_player = this.get_cells_occupied(this.player_grid_data), this.cells_occupied_by_placeholder = this.get_cells_occupied(this.placeholder_grid_data), this.last_cols = [], this.last_rows = [], this.collision_api = this.$helper.collision(d, this.options.collision), this.$preview_holder = a("<" + this.$player.get(0).tagName + " />", {
        ...
        var c = b.pointer.diff_left,
            d = b.pointer.diff_top,
            e = this.options.widget_base_dimensions[0],
            f = this.options.widget_base_dimensions[1],
            g = 1 / 0,
            h = 1 / 0,
            i = Math.ceil(c / (this.options.widget_base_dimensions[0] + 2 * this.options.widget_margins[0]) - .2),
            j = Math.ceil(d / (this.options.widget_base_dimensions[1] + 2 * this.options.widget_margins[1]) - .2),
            k = Math.max(1, this.resize_initial_sizex + i),
            l = Math.max(1, this.resize_initial_sizey + j);
        ...
        var m = {};
        !this.resize_dir.bottom && (m.width = Math.min(this.resize_initial_width + c, g)), !this.resize_dir.right && (m.height = Math.min(this.resize_initial_height + d, h)), this.$resized_widget.css(m), (k !== this.resize_last_sizex || l !== this.resize_last_sizey) && (this.resize_widget(this.$resized_widget, k, l, !1), this.$resize_preview_holder.css({
            width: "",
            height: ""
        }).attr({
            "data-row": this.$resized_widget.attr("data-row"),
            "data-sizex": k,
            "data-sizey": l
        })), this.options.resize.resize && this.options.resize.resize.call(this, a, b, this.$resized_widget), this.resize_last_sizex = k, this.resize_last_sizey = l
        ...
        for (var a, b = this.gridmap, c = [], d = [], e = b.length - 1; e >= 1; e--)
        ...
        for (var b = this.cols, c = b + (a || 1), d = b; c > d; d++)
